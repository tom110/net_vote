...
def getfreeproxy():
    ...
    try:
        response = request.urlopen(req, timeout=10)
        reResult = re.findall(r'(?:(?:[0,1]?\d?\d|2[0-4]\d|25[0-5])\.){3}(?:[0,1]?\d?\d|2[0-4]\d|25[0-5]):\d{0,5}',
...
# 免费http://pubproxy.com/api/proxy返回函数
def getfreejsonproxy():
    url = iniParser.pubproxy_com_api_proxy
    req = request.Request(url)
    req.add_header("User-Agent",
                   "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3080.5 Safari/537.36")
    try:
        ...
        reResult = jsonObject['data'][0]['ipPort']
        return reResult
    except:
        print("proxy获取失败")
        return ""
...
# 写数据进程执行的代码:
def write_freejsonproxy(q):
    print('免费json代理进程%s开始写入代理池' % os.getpid())
    while True:
        len = q.qsize()
        if len > 10:
            time.sleep(5)
            ...
            proxy = getfreejsonproxy()
            print('免费json代理地址%s写入代理池,当前线程池长度%d' % (proxy, len))
            q.put(proxy)
            time.sleep(0.5)
...
def main(argv):
    ...
    write_free_json_proxys_number=0
    try:
        opts, args = getopt.getopt(argv, "hf:j:e:d:r:",
                                   ["isusefileproxy=", "freejsonproxynumber=", "freeproxynumber=", "daxiangproxynumber",
                                    "readproxynumber"])
    except getopt.GetoptError:
        print(
            'mutiprocess.py -f <isusefileproxy>[False,True] -j <freejsonproxynumber>  -e <freeproxynumber>[int] -d <daxiangproxynumber>[int] -r <readproxynumber>[int]')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print(
                'mutiprocess.py -f <isusefileproxy>[False,True] -j <freejsonproxynumber>[int] -e <freeproxynumber>[int] -d <daxiangproxynumber>[int] -r <readproxynumber>[int]')
            sys.exit()
        elif opt in ("-f", "--isusefileproxy"):
            write_file_proxys_bool = arg
        elif opt in ("-j", "--freejsonproxynumber"):
            write_free_json_proxys_number = arg
    ...
    print('使用文件代理：', write_file_proxys_bool)
    print('免费json代理进程数：', write_free_json_proxys_number)
    ...
    # 开启免费json写入进程
    for i in range(int(write_free_json_proxys_number)):
        pw = Process(target=write_freejsonproxy, args=(q,))
        pw.start()
